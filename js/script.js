...
function handleMovement(circle, stationaryCircle, keys = {}, normalizedSpeed) {
  const originalPos = new SAT.Vector(circle.pos.x, circle.pos.y);
  const newPos = new SAT.Vector(circle.pos.x, circle.pos.y);

  // Vertical movement
  if (keys.w) newPos.y -= normalizedSpeed;
  if (keys.s) newPos.y += normalizedSpeed;
  ...
  // Horizontal movement
  if (keys.a) newPos.x -= normalizedSpeed;
  if (keys.d) newPos.x += normalizedSpeed;
...
let lastFrameTime = Date.now(); // Track the time of the last frame

function update() {
  const now = Date.now();
  const deltaTime = (now - lastFrameTime) / 1000; // Time elapsed in seconds
  lastFrameTime = now;

  const normalizedSpeed = speed * deltaTime; // Adjust speed based on deltaTime
